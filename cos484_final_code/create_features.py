...
import yaml
import torch
from feature_factory import RegexInterpreter, BertInterpreter, ElectraInterpreter, RobertaInterpreter, obtain_bert, obtain_electra, obtain_roberta
from feature_factory import WordToVecInterpreter
...
def main():
    ...
    elif interpreter_config['type'] == 'electra':
        electra_model, tokenizer = obtain_electra(interpreter_config['path'])
        use_logits = interpreter_config['use_logits'] == 'True'
        interpreter = ElectraInterpreter(electra_model, tokenizer, args.device, use_logits)
    elif interpreter_config['type'] == 'wordtovec':
        interpreter = WordToVecInterpreter() 
    elif interpreter_config['type'] == 'roberta':
        roberta_model, tokenizer = obtain_roberta(interpreter_config["path"])
        use_logits = interpreter_config['use_logits'] == 'True'
        interpreter = RobertaInterpreter(roberta_model, tokenizer, args.device, use_logits)   
    elif interpreter_config['type'] == 'electramed':
        tokenizer = AutoTokenizer.from_pretrained("giacomomiolo/electramed_base_scivocab_1M")
        model = AutoModelForPreTraining.from_pretrained("giacomomiolo/electramed_base_scivocab_1M")
        use_logits = interpreter_config['use_logits'] == 'True'
        interpreter = RobertaInterpreter(model, tokenizer, args.device, use_logits)   
    else:
    ...
    torch.save(train_features, os.path.join(save_dir, 'train.bin'), pickle_protocol=4)
    torch.save(dev_features, os.path.join(save_dir, 'dev.bin'), pickle_protocol=4)
    torch.save(test_features, os.path.join(save_dir, 'test.bin'), pickle_protocol=4)
