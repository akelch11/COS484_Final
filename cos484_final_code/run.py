...
def eval(args, model, eval_dataset, verbose=True):
    ...
    preds = None
    out_label_ids = None
    for batch in tqdm(eval_dataloader, desc="Evaluating", position=0, leave=True, disable= not verbose):
        model.eval()
...
def train(args, model, train_dataset, dev_dataset, verbose=True):
    ...
    for _ in train_iterator:
        ...
        epoch_iterator = tqdm(train_dataloader, desc="Iteration", position=0, leave=True, disable= not verbose)
        losses = []
...
def main():
    args = get_args()
    paper_args = torch.load("checkpoints/spouse_expbert/training_args.bin")
    print(paper_args)

    args.hidden_dim = paper_args.hidden_dim
    args.num_layers = paper_args.num_layers
    args.dropout = paper_args.dropout
    # args.feature_dim = paper_args.feature_dim
    args.projection_dim = paper_args.projection_dim

    args.learning_rate = paper_args.learning_rate
    args.train_batch_size = paper_args.train_batch_size
    args.num_train_epochs = paper_args.num_train_epochs
    args.max_grad_norm = paper_args.max_grad_norm
    args.weight_decay = paper_args.weight_decay
    ...
    if args.hyperparam_config != '':
        ...
        if args.eval:
            dev_dataset = create_dataset(args, dev_name, args.dev_bin_file, False)
            # load model
            model.load_state_dict(torch.load(os.path.join(args.output_dir, 'weights.bin'), map_location=torch.device('cpu')))
            model.to(args.device)
            print("EVAL")
            print(dev_dataset)
        ...
        print("TEST SET:" + str(os.path.join(args.exp_dir, 'test.bin')))
        test_dataset = create_dataset(args, "test", os.path.join(args.exp_dir, 'test.bin'), False)
        model.load_state_dict(torch.load(os.path.join(args.output_dir, 'weights.bin'), map_location=torch.device('cpu')))
        model.to(args.device)
